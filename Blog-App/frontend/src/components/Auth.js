...
const Auth = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if(isSignup){
     ...
     .then(() => navigate("/blogs")) ; // localStorege is available globally in side the javascript
    }else{
      sendRequest()
      .then(data => localStorage.setItem("userId" ,data.user._id) )
      .then(() => dispatch(authActions.login())).then(() => navigate("/blogs")) ; // use dispatch before calling the data
    }
  }
  return (
    <div>
